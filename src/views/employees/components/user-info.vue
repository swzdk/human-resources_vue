<template>
  <div class="user-info">
    ...
    <el-row type="flex" justify="end">
      <el-tooltip class="item" effect="dark" content="打印员工信息" placement="right-end">
...
export default {
  ...
  methods: {
    async getPersonalDetail() {
      ...
      this.$refs.picUpload.fileList = [
        { url: this.formData.staffPhoto, upload: true }
      ]
    },
    async savePersonal() {
      const fileList = this.$refs.picUpload.fileList
      if (fileList && fileList.some((item) => !item.upload)) {
        this.$message.warning('此时还有头像未上传成功！')
        return
      }
      await updatePersonal({
        ...this.formData,
        id: this.userId,
        staffPhoto: fileList && fileList.length > 0 ? fileList[0].url : ''
      })
